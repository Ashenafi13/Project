...
$sql = "SELECT user.Full_name,user.Student_ID,user.Institute,user.Program,student_request.Reason,student_request.Last_Class_Attend,student_request.Request_type,request_approve.Approved_By,offices.office_name,request_approve.Approved_Date,request_approve.Remark from user INNER JOIN student_request ON user.Student_ID=student_request.Student_ID  INNER JOIN request_approve ON student_request.Student_ID= request_approve.Student_ID INNER JOIN offices ON request_approve.Status_Office=offices.office_id where request_approve.Status='Rejected' and request_approve.Status_Office='8' ORDER BY Full_name ASC";
...
function Header()
{
    ...
    // Title
        ...
        $this->Cell(60);
        $this->Cell(120, 10, 'Clearance request rejected students report  ', 0, 0, " ", FALSE);
           $this->Ln(15);
}
...
$pdf->Cell(40, 20, "Rejected By", 1, 0, "", true);
$pdf->Cell(40, 20, "Office Rejected", 1, 0, "", true);
$pdf->Cell(50, 20, " Rejected Reason", 1, 0, "", true);
$pdf->Cell(40, 20, "Rejected Date", 1, 0, "", true);
...
while ($row = mysqli_fetch_assoc($r)) {
    ...
    $pdf->Cell(40, 10, "{$row['office_name']}", 1, 0, "", true);
    $pdf->Cell(50, 10, "{$row['Remark']}", 1, 0, "", true);
